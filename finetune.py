...
def main(p):
    ...
    # Load pretrained ViT-B on imagenet & reinit classification head
    model, state_dict_imagenet, old_state  = load_pretrained_vit_weights(model)
    reinit_classification_head(model)

    model_key = 'encoder_stack.encoder_stack.11.mhsa.w_q.weight'
    print("Same shape:", old_state['encoder.layers.encoder_layer_11.self_attention.in_proj_weight'].shape,
          model.state_dict()[model_key].shape)

    # See if Q-part of pretrained weight matches your tensor
    #qkv = old_state['encoder.layers.encoder_layer_11.self_attention.in_proj_weight']
    #q_part = qkv.chunk(3, dim=0)[0]  # first third = Q
    #print(torch.allclose(q_part, model.state_dict()[model_key]))  # True means perfect match

    check_loaded_layers(model, state_dict_imagenet)
    compare_state_dicts(model, state_dict_imagenet)
    compare_weight_distributions(
        pretrained_dict=old_state,
        model=model,
        pretrained_key='encoder.layers.encoder_layer_11.self_attention.in_proj_weight',
        model_key='encoder_stack.encoder_stack.11.mhsa.w_q.weight'
    )
    ...
    # Saving snapshots of the weights for later animations
    snapshots = defaultdict(list)

    interesting_layers = {
        "patch_proj": model.input_layer.patch_embeddings.proj.weight,
        "encoder_q_0": model.encoder_stack.encoder_stack[0].mhsa.w_q.weight,
        "encoder_q_mid": model.encoder_stack.encoder_stack[len(model.encoder_stack.encoder_stack)//2].mhsa.w_q.weight,
        "encoder_mlp_last": model.encoder_stack.encoder_stack[-1].feed_forward_layer.mlp[0].weight
    }
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # Save snapshots (list of weight distributions)
        if epoch % 1 == 0:
            with torch.no_grad():
                for name, layer in interesting_layers.items():
                    snapshots[name].append(layer.detach().cpu().flatten().numpy())
        # Save Snapshots
        torch.save(dict(snapshots), f'./runs/vit_weight_snapshots_{epoch+1}.pt')
        print(f'[*] Saved weight snapshots to vit_weight_snapshots_{epoch+1}.pt')
